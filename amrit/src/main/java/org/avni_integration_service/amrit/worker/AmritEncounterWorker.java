...
public class AmritEncounterWorker implements ErrorRecordWorker {
    ...
    public void processEncounters(AmritEntityType entityType) {
        while (true) {
            ...
            GeneralEncountersResponse response = avniEncounterRepository.getGeneralEncounters(status.getReadUptoDateTime(), SUBJECT_TYPE);
    ...
    protected void processEncounter(GeneralEncounter encounter, boolean updateSyncStatus, AmritEntityType entityType) {
        ...
        if(entityType.equals(AmritEntityType.BornBirth)) {
            bornBirthService.createOrUpdateBornBirth(beneficiary, encounter);
        } else if(entityType.equals(AmritEntityType.CBAC)) {
